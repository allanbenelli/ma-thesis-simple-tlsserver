...
use std::fs;
use std::io;
use std::io::{BufReader, Read, Write, Result as IOResult};
use std::net;
...
impl OpenConnection {
    ...
    fn send_http_response_once(&mut self) {
        ...
        */
        ...
        println!("response: {} from token: {:?}", cnt, self.token);
        let res = get_file_contents("sample").unwrap();
        let response = prepare_response("HTTP/1.1 200 OK", Headers::new(), res).unwrap();
        //println!("writing file to response: {:?}", String::from_utf8(&response).unwrap());
        //let res2 =             b"HTTP/1.0 200 OK\r\nConnection: close\r\n\r\nHello world from rustls tlsserver\r\n";
